...
export default function PropertySearchBar({ onSearch }: PropertySearchBarProps) {
  ...
  const handleSuggestionClick = (suggestionLocation: string) => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (highlightedIndex >= 0 && highlightedIndex < suggestions.length) {
          handleSuggestionClick(suggestions[highlightedIndex].location);
        } else {
          handleSearch();
        }
        break;
                  ...
                  {suggestions.map((suggestion, index) => (
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        {suggestion.area && (
                          <span className="text-xs text-muted-foreground">Area: {suggestion.area}</span>
                        )}
                      </div>
                    </button>
                  ))}
